				...
				if ($("#innovateTypeEditForm").form("validate")) {
					...
					if(!$("#innovateTypeEditForm").form("validate")) {
						...
						$("#innovateTypeEditForm").form({
						    url:"InnovateType/" + $("#innovateType_innovateTypeId_edit").val() + "/update",
						    onSubmit: function(){
								if($("#innovateTypeEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	innovateType_manage_tool = {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "InnovateType/" + rows[0].innovateTypeId +  "/update",
					type : "get",
					data : {
